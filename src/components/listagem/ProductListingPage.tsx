...
function parsePrice(val: number | string | null | undefined): number {
  if (!val) return 0;
  if (typeof val === "number") return val;
  return parseFloat(
    val.replace("R$", "").replace(".", "").replace(",", ".").trim(),
  );
}
...
export function ProductListingPage({
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (filters.materiais && filters.materiais.length > 0) {
      list = list.filter((p) => filters.materiais.includes(p.material));
    }
    if (filters.terrenos && filters.terrenos.length > 0) {
