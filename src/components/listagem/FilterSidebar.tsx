...
export function FilterSidebar({ products = [], filters, onChange, onClear }: FilterSidebarProps) {
  ...
  const dynamicFilters = useMemo(() => {
    ...
    return {
      ...
      tamanhos: (Array.from(tamanhosSet) as number[]).sort((a: number, b: number) => a - b),
      materiais: (Array.from(materialSet) as string[]).map((m: string) => ({ id: m, label: m })),
      terrenos: (Array.from(terrenoSet) as string[]).map((t: string) => ({ id: t, label: t })),
    };
  }, [products]);
    ...
    <aside className="w-full font-barlow lg:sticky lg:top-24">
      ...
      {dynamicFilters.materiais.length > 0 && (
        <FilterSection title="Material" defaultOpen={false}>
          <CategoryFilter
            options={dynamicFilters.materiais as { id: string; label: string }[]}
      ...
      {dynamicFilters.terrenos.length > 0 && (
        <FilterSection title="Tipo de Terreno" defaultOpen={false}>
          <CategoryFilter
            options={dynamicFilters.terrenos as { id: string; label: string }[]}
