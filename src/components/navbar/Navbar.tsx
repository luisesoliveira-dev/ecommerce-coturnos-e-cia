...
export interface NavbarProps {
  forcedCartOpen?: boolean;
  setForcedCartOpen?: (open: boolean) => void;
}

export function Navbar({ forcedCartOpen, setForcedCartOpen }: NavbarProps) {
            ...
            <ul className="hidden lg:flex gap-6 xl:gap-8 list-none m-0 p-0 items-center h-full">
              ...
              {linksNavbar.map((link) => (
                <li key={link.label} className="flex items-center h-full">
                  <Link
                    to={link.href}
                    className="relative text-army text-sm font-semibold uppercase tracking-[1.5px]"
                  >
                    {link.label}
                  </Link>
                </li>
